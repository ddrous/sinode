...
inner_tol_node=1e-10
inner_tol_coeffs=1e-9
...
##%%
# Define the Lorenz-like system
def lorenz(t, state, a, b, c):
    x, y, z = state
    dxdt = -(a*b)*x/(a+b) - y*z +c
    dydt = a*y + x*z
    dzdt = b*z + x*y
...
init_conds = np.array([[-0.5, -1], [-0.5, -0.5], [-0.5, 0.5], [-0.5, 1], 
                       [-1, -1], [-1, -0.5], [-1, 0.5], [-1, 1], 
                       [-2, -1], [-2, -0.5], [-2, 0.5], [-2, 1],
...
init_conds = [np.array([x, y, 0.0]) for x, y in init_conds]


# init_conds = []
# for x in np.linspace(-1/10, 1, 3):
#     for y in np.linspace(-1, 1, 3):
#         for z in np.linspace(-0, 1, 1):
#             init_conds.append([x, y, z])
...
for i, state0 in enumerate(init_conds[::1]):
    # state0_full = [state0[0], state0[1], 0.0]
    # print("Initial conditions: ", state0)
    sol = solve_ivp(lorenz, t_span, state0, args=(a, b, c), t_eval=t_eval)
    train_data.append(sol.y.T)

    ## Plot the phase space
    ax = sbplot(sol.y[2], sol.y[0], ".", color=colors[i], ax=ax, dark_background=True)

    ## Initial conditions
    ax = sbplot(state0[2], state0[0], "X", color=colors[i], ax=ax, dark_background=True, markersize=10)
...
model = NeuralODE(data_size=3, dict_size=dict_size, mlp_hidden_size=mlp_hidden_size, mlp_depth=mlp_depth, key=model_keys[0])
coeffs = Coefficients(data_size=3, dict_size=dict_size, key=model_keys[1])
...
def proximal_training(model, coeffs, eval_prop=1.0, init_lr=1e-5):
    ...
    eval_length = int(eval_prop * len(t_eval))
    train_key, _ = jax.random.split(main_key)
    ...
    for out_step in range(nb_outer_steps_max):
        ...
        for in_step_node in range(nb_inner_steps_max):
            ...
            for i in range(0, nb_data_points, batch_size):
                ...
                batch = (data[0,i:i+batch_size,:eval_length,:], t_eval[:eval_length])
    ...
    return model, coeffs, losses_node, losses_coeffs
...
for (ep, lr) in zip((0.1, 0.5, 1.), (1e-4, 1e-6, 1e-7)):
    print(f"Training for eval_length: {ep} and lr: {lr}")
    model, coeffs, losses_node, losses_coeffs = proximal_training(model, coeffs, ep, lr)
...
eqx.tree_serialise_leaves("data/sinode_model_06.eqx", model)
eqx.tree_serialise_leaves("data/sinode_coeffs_06.eqx", coeffs)



# %%

# model = eqx.tree_deserialise_leaves("data/sinode_model_05.eqx", model)
# coeffs = eqx.tree_deserialise_leaves("data/sinode_coeffs_06.eqx", coeffs)


## %%
eval_length = int(1e-1*len(t_eval))
...
for state0 in init_conds_test:
    state0_full = [state0[0], state0[1], 0.0]
    sol = solve_ivp(lorenz, t_span, state0_full, args=(a, b, c), t_eval=t_eval)
    test_data.append(sol.y.T)
test_data = np.stack(test_data)[None, ...]


X = test_data[0, :, :eval_length, :]
# t = np.linspace(t_span[0], t_span[1], T_horizon)
t = t_eval[:eval_length]
...
for i in range(1):
    ...
    # new_lambdas = saved_gammas
    # new_gammas = saved_lambdas

    # new_lambdas = new_lambdas.at[i].set(0.)
    # new_gammas = new_gammas.at[i].set(0.)
    ...
    for i in range(X.shape[0]):
        if i==0:
            sbplot(X_hat[i, :,dim0], X_hat[i, :,dim1], "-", x_label='x', y_label='y', label=f'Pred', title=f'Phase space', ax=ax, alpha=0.5, color=colors[i])
            sbplot(X[i, :,dim0], X[i, :,dim1], "+", lw=1, label=f'True', ax=ax, color=colors[i])
        else:
            sbplot(X_hat[i, :,dim0], X_hat[i, :,dim1], "-", x_label='x', y_label='y', ax=ax, alpha=0.5, color=colors[i])
            sbplot(X[i, :,dim0], X[i, :,dim1], "+", lw=1, ax=ax, color=colors[i])
...
print("Vector field at [1,1]: \n", evaluate_funcs_dir(model.vector_field.basis_funcs, jnp.array([1., 1., 1.])))
print("Vector field at [2,2]: \n", evaluate_funcs_dir(model.vector_field.basis_funcs, jnp.array([2., 2., 2.])))
