...
gen_data = True
run_folder="runs/240515-084130/" if not gen_data else None
...
# params3 = 0., 0., 1/25.
# params4 = -1/2., -2, 2/10.
# params5 = -1/1., -0.1, 1/50.

# params1 = -1/2., -1, 1/10.
a_s = np.linspace(-0.75, -0.25, 2)
b_s = np.linspace(-1.15, -0.85, 2)
c_s = np.linspace(0.05, 0.15, 2)


t_span = (0, T_horizon)
t_eval = np.arange(t_span[0], t_span[1], 0.01)[::skip]

init_conds_train = np.array([
...
if gen_data == True:
    ...
    # envs = (params1, params2, params3, params4)
    # envs = (params1, params2, params3, params4, params5)
    # envs = (params1, params2, params3)
    envs = [(a, b, c) for a in a_s for b in b_s for c in c_s]

    nb_envs = len(envs)
    fig, axs = plt.subplots(nb_envs, 1, figsize=(6, 2.5*nb_envs), sharex=True)

    test_data = np.zeros(shape=(nb_envs, len(init_conds_test), len(t_eval), 2))
    for i, state0 in enumerate(init_conds_test):

        for e, params in enumerate(envs):
            sol = solve_ivp(duffing, t_span, state0, args=params, t_eval=t_eval)
